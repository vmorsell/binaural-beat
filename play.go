...
func Play(freq, beatFreq float64) {
	portaudio.Initialize()
	defer portaudio.Terminate()

	go play(freq, beatFreq)
...
func play(freq, beatFreq float64) {
	...
	// Initialize an oscillator inducing a binaural beat of the desired frequency.
	o, err := NewOscillator(freq-beatFreq/2, freq+beatFreq/2, sampleRate)
	if err != nil {
